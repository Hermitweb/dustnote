...
tagsRouter.post('/note-tags', (req, res) => {
  ...
  const note = db.prepare('SELECT 1 FROM notes WHERE id = ? AND user_id = ?').get(parsed.data.noteId, user.userId);
  const tag = db.prepare('SELECT 1 FROM tags WHERE id = ? AND user_id = ?').get(parsed.data.tagId, user.userId);
  if (!note || !tag) {
    res.status(404).json({ error: 'not_found' });
    return;
  }
  try {
    db.prepare('INSERT INTO note_tags (note_id, tag_id) VALUES (?, ?)').run(parsed.data.noteId, parsed.data.tagId);
