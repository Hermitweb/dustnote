...
- **全新布局（Web）**：216px 导航轨（视图 / 文件夹树 / 标签常驻）+ 舞台三态
  （概览 → 列表 → 详情，`Esc` 逐级回退、`Ctrl+K` 搜索、结果集内 `←/→` 翻篇）；
  未选中笔记时不再是整屏空白，改为**概览首屏**（统计 + 最近编辑 + 快捷入口）
- **主题引擎三端同源**：7 套皮肤改由主题 seed 派生（shared 单一真源），
  默认主色定稿雾霭蓝调；小程序 token 由脚本生成，三端配色结构性一致；
  时间格式统一走 shared（修 iOS/JSC 解析差异的根）
- **图标体系**：界面标签 emoji 全部剥离为真图标，check-i18n 新增
  「词典禁 emoji」硬规则门禁；玻璃材质收敛为三档且设置内可关
- **阅读纸张**：正文 68ch 版式 + 分屏双栏；1280 以下自动收图标轨 / 抽屉导航

### 测试与工程

- **三端测试清零 `echo [skip]`**：miniprogram（42 例）、desktop（Tauri mock
  基建）、mobile 主题测试落地；全仓 479+ 用例、7 包 lint 0 警告
- e2e 新增 **a11y 对比度门禁**与视觉基线 spec；官方端点文档统一为
  `napi.iniess.cn`（HTTPS 真证书）
...
- **自动诊断采集**：应用内未捕获错误与关键网络失败现在会脱敏（剥离 URL、
  截断长度、不含笔记内容）后排队回传到你自己的服务器，回前台自动补投——
  排障不再依赖连接数据线抓日志。服务器提供鉴权后的查看/清空端点
  （`GET /api/v1/diagnostics`），上报通道保留 30 天自动修剪

### 内部

- 发版工具链落地：`scripts/bump-version.mjs`（26 文件版本清单程序化 +
  全仓残留自检）与 `deploy/upgrade.sh`（服务器一键升级：label 反查旧目录、
  零停机构建顺序、卷迁移、downloads 三件套同步、健康版本断言）
- 诊断接收端点为匿名可达 + 独立限流（崩溃多发生在登录完成之前）

## [2.5.43] - 2026-09-24

### 修复（全端）

- **服务器不可达时永久卡在鉴权检查**：安卓联机探测失败后界面永久转圈、
  网络恢复不自愈（5s 兜底超时被提前清除）；现显示「无法连接服务器」
  错误页 + 重试按钮，回到前台自动重探。小程序同款问题一并修复
  （不再永显「加载中」，自动退避重试 + 手动重试入口）
- **安卓解锁页/列表反复静默重挂载**：屏幕注册处每次渲染生成新组件，
  导致当前页反复卸载重挂并重放全部网络请求（弱网/同步频繁时肉眼可见
  抖动、耗流）；已提升为稳定组件类型
- **网络错误不再直出英文技术原文**（signal is aborted without reason /
  Failed to fetch 等）：解锁失败、加载失败、解锁提交失败等提示统一改为
  「无法连接到服务器」等语义化文案（安卓/Web/小程序三端，Web 桌面端
  复用 Web 同步生效）
- Web 端 API 请求补充 30 秒超时，断连黑洞不再长时间停在加载页

### 内部

- 服务器探测失败状态位与错误文案分桶进入跨端审计台账（bf1f275/fa0da1a/0421d25）
...
- **解锁页标题跟随语言**：解锁页/单机解锁页大标题与指纹弹窗提示此前在
  中文环境仍显示英文 DustNote——改为「尘渊笔记」（英文环境不变），与
  小程序端 2.5.39 同语义
- **生物识别解锁跳转提速**：指纹/面容通过后不再等待网络刷新 token——
  立即恢复持久化凭据切解锁态，过期由 401 静默刷新自愈；此前弱网/代理
  环境下验证通过后会在解锁页停留数秒
- 解锁页生物识别按钮即时切换「解锁中…」反馈态并防重复点击（联机/
  单机两处统一）

## [2.5.39] - 2026-09-10

> 全端深度审计修复版：严重×1（静默丢数据）、高×8、中×18 及约 20 项低危
> 全部闭环，服务端/web/desktop/client-core/miniprogram/mobile 共 56 文件。

### 修复（数据安全 · 严重/高）

- **离线队列 429 不再丢数据**（三端）：写限流触发时此前被当作"不可恢复
  4xx"永久丢弃——离线攒批重放超过 60 条/分钟即静默丢失后续笔记。现归入
  可重试（指数退避），服务端写限流同步放宽到 300/分钟
- **服务端导出端点坏死**（H1）：/account/export 引用了 notes 表不存在的
  created_at/updated_at 列，100% 返回 500；已修复并新增"真实迁移产物建库"
  的 schema 漂移回归测试
- **失效分享清理从未生效**（H2）：NOT NULL 约束使密文清除 UPDATE 必然失败，
  且"当日无可清理笔记时提前返回"使清理任务静默跳过——双重根因修复，
  改为失效超 30 天整行删除（服务端最小化留存承诺兑现）
- **大库笔记消失**（H3，三端）：GET /notes 单页上限 500 且无续传，笔记
  超过 500 条后最旧的从 UI 消失——服务端新增游标分页（hasMore/nextCursor），
  web/安卓/小程序 loadAll 全部循环拉取至尾页
- **笔记创建幂等**（H4）：弱网重放同 id 二次提交不再 500×8 次后被队列丢弃，
  服务端 ON CONFLICT 收敛返回现状
- **幽灵笔记根治**（H7）：命令面板/快捷键/快速记事/模板创建在未选文件夹时
  本地与服务端归属分叉；移动弹窗移除「未分类(null)」陷阱选项
- **批量模式全选误伤**（H6）：「全部」视图树形模式下全选曾可盲选全库批量
  删除——现限定当前文件夹范围
- **文件夹删除级联闭环**（H8）：删除前确认弹窗（含波及笔记数）、本地一次性
  级联清理后代文件夹、单机库同步级联；侧栏新增「未分类」节点使无归属
  笔记重新可见可达
- **Service Worker 不再缓存 API 响应**（H5）：明文文件夹名与 wrappedMasterKey
  曾被写入 Cache Storage 且无清理路径；现完全绕过 /api/，离线体验由加密
  IndexedDB 缓存承担

### 修复（安全加固）

- 2FA 开启/关闭端点换用防重放 TOTP 校验（M7，对齐解锁路径）
- 软删除笔记时事务内吊销其全部活跃分享链接（M6）
- update-manifest sha256 缓存按 mtime+size 失效，同版本覆盖产物不再给出
  旧 hash（M1）
- 多标签页并发刷新用 Web Locks 串行化，不再互踢回解锁页（M12）
- 移动端 refresh token 从 AsyncStorage 明文迁 Keychain（M10）；公网明文
  HTTP 服务器地址配置时提示风险（M16）
- WYSIWYG 富文本进入 TipTap 前过 DOMPurify 白名单（M15）
- 桌面更新白名单信任模型文档化（M11）；compose 容器内存上限 1g（M8）；
  nginx /downloads/ 独立限流；audit_log 180 天自动修剪（M3）；破坏性迁移
  强制当日备份门槛（M4）；优雅停机强断残留连接

### 修复（体验）

- 桌面端编辑器内右键菜单恢复（剪切/复制/粘贴此前被全局拦截，M17）
- 批量操作失败条数明确告知（此前静默吞掉，M14）；移动/恢复补 409 自愈重试
- 切换模式时清理离线队列/缓存/旧 refresh token（M13）
- 小程序模式迁移失败条数如实披露并自动重试，重试不再重复建文件夹（C1b/M9）
- ShareDialog 网络失败给出提示；首次访问默认联机副作用的 StrictMode 双跑

### 工程化

- SW_VERSION 与应用版本一致性纳入 CI 门禁（此前停滞 4 个版本未推进）
- compose/.env.example 版本默认值随发版对齐（默认部署不再静默漏产物，M5）
- 运维文档校准：runbook/self-hosting 的备份与监控章节对齐实际实现，
  移除虚构的 Prometheus/backup.sh 描述；DEPLOY.md 框架名纠正
- WebAuthn 死代码与依赖移除；DELETE 软删幂等化；列表查询补索引

## [2.5.38] - 2026-09-08

### 新增（小程序 · 对齐安卓端）

- **联机恢复码找回密码**：解锁页新增「忘记密码？」入口，恢复码+新主密码
  三步找回（recovery-params / recover / rewrap），原 masterKey 不变，
  历史笔记不受影响
- **指纹解锁（微信 SOTER）**：设置页开关（开启需当次指纹验证），解锁页
  指纹按钮直达；前置双检（支持矩阵+已录入）；密码解锁成功自动续写缓存，
  手动锁屏不清缓存；存储弱于 Keystore 的取舍已由产品确认接受
- **分享管理**：复制/分享访客可直开链接（本地解封 shareKey）、列表显示
  笔记标题、展示过期时间/永久有效
- **编辑器**：底部菜单新增「应用模板」（预设模板插入正文）；历史版本
  点击可预览 600 字内容，确认后直接恢复
- **导出 Markdown / 从文件导入 .md/.txt / 粘贴 JSON 导入**（与安卓端
  格式互认：# 标题 / > 标签： / --- 分隔）
- **模式切换数据迁移**（DM-7 延迟迁移+重加密）：切换前导出备份并暂存
  旧 masterKey，新模式首次鉴权成功后自动重加密导入，导入失败下次解锁
  自动重试——不再出现迁移后全部 🔒
- **列表下拉刷新**：首页/回收站/分享管理/文件夹管理四个列表页

### 修复（小程序 · 数据安全）

- **解密失败不再覆盖原密文**：解密失败置位后禁止一切保存路径（此前
  「解密失败」标题+空正文会被自动保存加密提交，永久覆盖原笔记）
- 自动保存 dirty 守卫（消灭打开笔记后必触发的冗余 PATCH/version+1）
  - 离开页面冲刷防抖窗口内未保存修改
- 单机模式修改密码轮换恢复码（旧码失效）并展示+复制新码
- 离线队列重放失败不再产生未处理拒绝上报（request:fail timeout 类）
- FAB 新建笔记网络失败自动入离线队列；首页加载失败空态可重试

### 交互统一（全端标准核查）

- 输入框聚焦时隐藏占位提示：web 全局 CSS、安卓 26 处 TextInput 换
  FTextInput、小程序 20 处换 FInput（核查确认 web/安卓无收藏双星、
  重复返回键、深色页头白条问题）
- 移除编辑/设置/文件夹/回收站/分享管理页内重复返回箭头（统一走原生
  导航栏返回，标题居中）
- 深色模式页内标题栏白条：毛玻璃覆写按主题着色（webview 同样命中
  @supports 查询，此前注释误标 H5 专享）
- 首页收藏按钮双星、文件夹管理 ➕ 点击反馈增强、首页文件夹筛选改
  路径式下钻 + 记住上次选中、文件夹管理目录树 + 行内操作（对齐安卓端）

## [2.5.37] - 2026-09-07

### 修复（小程序）

- **笔记列表恒空**：2.5.34 搜索重构时丢失「无搜索词放行全部」守卫，
  searchRank 空 Map 导致所有笔记被过滤——列表永远空白（服务器数据
  实际已正常返回，纯客户端过滤层问题）
- **页头颜色与应用主题相反**：darkmode 开启但缺 theme.json，微信原生
  导航栏按自己的默认色跟随系统，与页面内容主题（应用偏好）脱钩；
  补 theme.json 成对配色 + ThemeVars 全分支挂载（此前锁定/加载早退
  分支不渲染，导航栏无人设置）+ 导航栏颜色延迟重试与 weapp 原生
  wx 直调（Taro 封装在部分启动时序下静默丢调用）
...
- 笔记列表长按弹出操作菜单：置顶 / 收藏 / 分享（联机）/ 批量选择，
  与 web 右键菜单对齐；此前长按只能进多选，列表层无任何单条操作入口
- 操作失败改为弹窗提示（此前静默失败体感为「点了没反应」）

**小程序**

- 列表行操作区补齐：置顶切换、分享（联机，一键创建分享并复制链接；
  密码/有效期等进阶选项仍在编辑页分享弹窗）

**桌面**

- 更新白名单按 URL origin 归一化：修复反代域名（或端口书写差异）下
  应用内更新报「非法的下载地址」；放行 manifest 产物自身 origin

### 修复

- 服务器 update-manifest 产物地址改由 WEB_ORIGIN 决定并对齐反代域名
  （napi.iniess.cn），存量 2.5.34/2.5.35 客户端无需重装即可应用内更新
...
- 笔记右键菜单新增「收藏 / 取消收藏」「置顶 / 取消置顶」「分享」（联机模式
  打开编辑器分享对话框）
- 快捷键补齐：Ctrl+N 新建笔记、Ctrl+S 立即保存（web 端启用，覆盖浏览器
  默认行为）；Delete 将当前选中笔记移入回收站（带确认）

### 修复

- 幽灵笔记根治：命令面板/快捷键新建笔记时若未指定文件夹，自动归入第一个
  文件夹（此前产生无归属笔记，列表不可见）

## [2.5.34] - 2026-09-05

### 修复（全平台功能审计修补——三代理交叉审计发现）

**Web/桌面**

- **token 15 分钟过期风暴**：API 401 → 单飞 /auth/refresh(cookie)→ 新 token
  重放原请求；刷新失败回解锁页并清密钥——修批量操作/历史版本/设备管理
  的「token 无效或已过期」连环报错
- 离线队列 401/403 不再丢弃待同步操作（防数据丢失）
- 编辑器工具栏 flex-wrap：窄窗口自动换行不挤压
- 桌面分享链接改用服务器地址（tauri origin 链接访客无法打开）
- 分享创建失败给出错误提示（此前静默）；分享密码输入框改密码型

**小程序**

- 修复 3 页 hooks 顺序违规（分享页每次成功加载必崩、列表/模式选择状态
  跃迁可崩）
- 模板/文件夹选择弃用 showActionSheet（6 项硬上限，7 个预设模板必失败），
  改用无上限的半屏选择面板
- 回收站笔记纳入列表明文（trash 标签标题与搜索不再全空）
- 深色模式补 .theme-light 反制类（手动浅色+系统深色不再拼色错乱）
- 搜索提出过滤循环（500 笔记场景从每键 500 次检索降为 1 次）
- common.select 等键归位（列表「选择」按钮此前显示字面量）

**Android**

- 语音听写双杀修复：回调注册方式改为赋值式（此前运行时必 TypeError）；
  最终识别结果正确映射正文追加（此前只显示不落正文）；错误态复位+
  页面卸载清理
- 联机写路径补乐观锁 version：置顶/收藏/移动文件夹/回收站恢复此前
  全部 400
- 历史版本恢复补 clientUpdatedAt（此前必 400）
- 直连保存 409 自愈：基线未变（仅版本落后）自动用新版本重放，
  修 1.5 秒循环弹窗
- 截屏偏好 SharedPreferences 持久化（重启/重建后不再回退为禁止）
- 离线队列旧密文（a=0）解密不再无条件传 AAD

## [2.5.33] - 2026-09-05

### 小程序功能补齐(追赶 Web 端)

- **分词全文搜索**:中文二元组倒排索引(weapp 无 Intl.Segmenter 的移植方案),
  标题加权、结果按相关性排序,替代原子串匹配
- **标签体系**:编辑页标签编辑器(逗号分隔);列表页标签筛选 chips
- **自定义模板**:联机拉取服务端自定义模板(密文存储、使用时解密);
  编辑页「存为模板」一键保存当前笔记为模板
- **WebSocket 实时同步**:监听 note_changed/share_changed,多端编辑 300ms
  防抖自动刷新(断线指数退避 5s→60s)
- **备份导入(weapp)**:从微信聊天记录选择备份 JSON 导入(联机跳过已存在,
  失败如实上抛);导入前二次确认
- **图片渲染**:Markdown 预览渲染 http(s) 图片;本地引用(dustnote-img://)
  显示可读占位(跨设备不同步的架构限制)

### 其他

- 全端版本号对齐 2.5.33(此前 2.5.31/32 仅小程序先行)

## [2.5.30] - 2026-09-04

### 修复（桌面端，回退 v2.5.29 的 WebView2 启动参数）

- **更新后窗口全透明占位**：v2.5.29 引入的 `additionalBrowserArgs`
  （禁用遮挡挂起/后台节流）在部分机器上导致 WebView2 完全不渲染——
  窗口存在、可聚焦、但零像素（全透明且阻挡点击）。全量回退该参数，
  恢复与 v2.5.28 一致的启动行为
- 「长时间空闲后窗口空白/透明」的原问题回归为已知问题，后续改用
  替代方案（wry 升级或恢复显示时重建 WebView）解决，不再通过
  additionalBrowserArgs 规避

### 其他

- 小程序 2.5.30 同步：联机鉴权竞态修复、深色模式手动切换、真机
  TextEncoder 垫片、ActionSheet 取消报错过滤
...
- **全平台设置自动回填服务器地址**：已联机时，web/desktop 设置弹窗「服务器地址」
  输入框、mobile 设置页新增展示行 + 切换模式弹窗与模式选择页、小程序设置行与
  模式选择页均自动填入当前实际连接地址，无需重打
- **mobile 语音听写**：编辑页 🎤 按钮（@react-native-voice/voice），实时转写追加正文
- **动线深度优化**：编辑预览防抖 300ms、反向链接索引 WeakMap 缓存（输入不再卡顿）、
  新建笔记记住上次所在文件夹、列表页模板新建入口

### 修复

- **web 批量选择入口状态机缺陷**：经按钮退出选择模式后残留全选状态，导致再次
  点「选择」被瞬间弹回、批量操作无法进入；改为进入默认全选/未全选补全/已全选退出
- **miniprogram 暗色主题跟随系统**：WXSS 不支持 CSS 变量切换，改用
  `@media (prefers-color-scheme: dark)` 注入深色变量；设置页主题切换在 weapp
  端给出跟随系统的说明提示
- 外部审计消化：Android debug 签名降级链补告警、CSP 边界声明、敏感响应
  Cache-Control no-store、宽限期口径收紧、测试治理

## [2.5.27] - 2026-09-03

### 修复（审计待办收尾）

- **desktop 更新链 fail-closed（双层）**：TS 层 hash 缺失拒绝下载执行；
  Rust 层下载改随机临时文件名 + create_new（防本机抢占/预置），启动安装器
  改经 opener 插件
- **web 图片预览接线**：预览/分屏渲染前还原 dustnote-img:// 引用
  （image-store 异步读 IndexedDB）——修复图片在预览中永不渲染
- **web 离线 409 三方合并接线**：updateNote 入队携带 conflictCtx
  （base=改前明文/local=编辑结果）——SyncEngine 字段级合并从死代码
  变为可用，离线编辑撞 409 不再静默丢弃
- shared 诊断兜底值 310000 → 100000（一致性）

## [2.5.26] - 2026-09-03

### 修复（审计待办消化）

- **server 安全**：TOTP 失败计入防爆破锁定并落库窗口计数器防重放；每日自动
  备份调度上线（此前备份卷从未被调度）+ 备份 API 补 await；失效/吊销分享
  30 天后清除密文；nginx body 上限对齐应用层（修大笔记 413）；COOKIE_SECURE
  env 支持非 HTTPS 部署显式关 Secure cookie
- **web/桌面**：loadAll 并发单飞 + 锁定中止（自动锁屏后明文不再驻留内存）；
  保存 409 自愈重试（本地密文+服务端最新 version），修复保存链路瘫痪；
  桌面更新 hash 缺失 fail-closed（TS 层）
- **mobile**：改密/恢复派生沿用账号 KDF 参数（防 rewrap 后跨端锁死）；自动
  保存 dirty 守卫；iOS 仅 background 锁定；refresh 失败分类处理
- **miniprogram**：importBackup 跳过回收站笔记；placeholder 文案去硬编码 IP

### 界面

- 模式选择页排版修正（输入框高度/标题/按钮居中）；解锁按钮提交态「解锁中…」
- 全端更名「尘渊笔记」（英文 DustNote 不变）；新小程序 AppID 正式启用

## [2.5.25] - 2026-08-31

### 变更：密码长度策略放宽

- **主密码最短 8 → 6 位**：三端注册/恢复/修改密码的全部校验点与文案
  （含 shared 单机认证共享校验、强度条最低档）；既有账号不受影响
- **分享密码最短 8 → 4 位**：服务端 CreateShareSchema 与 web/小程序
  预校验、4-64 提示文案同步
...
- **加密引擎接入（根治解锁分钟级耗时）**：quick-crypto 此前仅在依赖中声明，JS 侧从未调用 `install()`，RN 无 WebCrypto 导致密钥派生始终走纯 JS 回退（310k 迭代分钟级、派生期间界面冻结）。现已在应用入口接入原生 JSI 引擎并写入诊断状态——密码解锁降至 1 秒内，生物识别不再被派生阻塞
- **PBKDF2 迭代 310000 → 100000**（OWASP 2021 档位；新注册/新设主密码生效，存量账号按服务端记录参数派生）
- 批量操作：长按笔记进入多选，底部操作栏全选/移动/删除（软删除进回收站），移动目标弹层按文件夹列出
- 8 处裸 fetch 补 `X-Client` 头（修复笔记版本 http400、分享失败缺参数）

### 新特性：全平台本地化

- **web/桌面**：补齐组件层硬编码文案（35 处）、修复 6 个组件引用不存在词典 key 导致的裸 key 显示
- **Android**：全部页面接入 i18n（约 285 处），词典扩至 438 key
- **小程序**：从零搭建轻量 i18n（零依赖），13 个页面约 500 处双语化，设置页新增语言切换
- 词典中英双侧程序化比对对齐

### 新特性：双语产品名（中文「尘心笔记」/ 英文「DustNote」）

- web 标签页标题、桌面窗口标题/托盘 tooltip/系统通知跟随语言
- Android 中文系统桌面图标名「尘心笔记」，其他语言 DustNote
- 小程序导航栏标题跟随语言
- 安装包名称/元数据与桌面原生菜单保持英文；NSIS 安装向导语言跟随系统

## [2.5.20] - 2026-08-30

本版为安卓端体验改造批次：文件夹层级导航、文件夹管理页重做、加密引擎诊断。

### 改进（Android）

- **文件夹层级导航（路径式下钻）**：顶部面包屑（🏠 全部 ▸ 一级 ▸ 二级）可点任意一级回跳；下方展示当前层的子文件夹；笔记列表只显示当前层——类似文件路径逻辑，替代原"全部+平铺文件夹"的扁平筛选
- **排序分类精简**：移除时间/标题/字数分类（固定按更新时间倒序，置顶优先）
- **文件夹管理页重做**：每个文件夹占满一行（含子文件夹计数），右侧四个操作（➕ 添加子文件夹 / ✏️ 重命名 / 📁 移动 / 🗑️ 删除，均带命中区扩展）；点击行展开/折叠子文件夹；行内 ➕ 弹出创建子文件夹 Modal
- **解锁页加密引擎诊断**：底部小字显示加密引擎状态（原生 JSI / 兼容模式），用于定位密码派生耗时过长的根因（quick-crypto 回退嫌疑待实锤）
- 根层空态文案区分："进入文件夹查看笔记"（新规则下笔记归文件夹）

### 修复（Web/桌面）

- 侧栏新增「📋 全部笔记」视图返回入口（收藏/回收站 → 文件夹树）

## [2.5.19] - 2026-08-30

本版为体验与分发大版本：Windows 安装体系重构（NSIS + 自研应用内更新）、文件夹优先的产品规则，以及全平台问题对照修复。

### 新行为：文件夹优先

- **首次使用自动初始化**：创建默认文件夹「关于尘心笔记」+ 引导笔记（产品简介与使用方法，可编辑可删除）
- **笔记必须归属文件夹**：「未分类」分组从产品移除；历史未分类笔记自动迁入默认文件夹（迁移时 toast 提示）
- **创建笔记强制归文件夹**：Web 未选中文件夹时提示先选；Android「全部」视图拦截提示；小程序未选时 ActionSheet 必选（无文件夹自动初始化默认内容）
- **删除文件夹**时其中笔记移入默认文件夹（默认文件夹被删则下次自动重建），笔记永远有归属

### 修复

- **Windows 桌面包联机/更新全部不可用的两个根因**（真机 "Failed to fetch" 的真正来源）：
  1. Tauri 生产包 CSP `connect-src` 不含用户自填服务器地址 → WebView2 拒绝一切外联；放宽为允许任意 http(s)（自托管服务器地址本就任意）
  2. 服务端 CORS 白名单缺 Tauri v2 Windows 实际源 `http://tauri.localhost`；已补入
- **桌面应用内更新 SHA-256 必失败**：manifest hash 为占位空串却被客户端当作期望值比对；双侧归一（空串视为未提供跳过校验）
- **切换模式误报成功**：checkStatus 失败不抛错导致"先报成功、再跳错误页"；改为读取最终状态给出准确结果；确认弹窗文案如实描述"不自动迁移数据"（消除误清数据风险）
- **switchMode 设计缺口**：旧实现切联机时对未注册的远端执行清库/导入，必然网络失败回滚（"Failed to fetch"）；重设计为只切通道不自动迁移，原模式数据保留，迁移走导出/导入

### 新功能（桌面）

- **Windows 新增 NSIS 传统安装包**：使用许可声明页、安装路径选择、WebView2 环境检测/自动安装、控制面板卸载程序（PerUser/PerMachine 可选）
- **移除 Velopack，应用内更新自研内置**：检查走自建服务器 update-manifest（无 GitHub API 限流问题），下载走 GitHub Releases 直链（CDN 域不限流）+ SHA-256 校验 + 启动 NSIS 向导
- ⚠️ 迁移说明：旧 Velopack 安装（2.5.17/18 Setup.exe）不会被自动更新到本版，请从 Release 页下载 `*_x64-setup-nsis.exe` 覆盖安装（数据保留）；安装包产物有变化：不再产出 Setup.exe/MSI/RELEASES/nupkg

### 改进

- Android 恢复码页新增"复制恢复码"按钮（全局防截屏特性下替代截图保存）
- 分享密码客户端预校验统一为 8-64 位（与服务器一致，消除"设 4-7 位被拒"误导）
- 桌面单机模式下启动不再白跑更新检查；分享页密码/错误分支矮视口不再裁剪；Clipboard 换用项目统一库
