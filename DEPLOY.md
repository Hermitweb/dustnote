...
> 指定版本：追加 `--version v2.5.34`（Linux/macOS）或 `-Version v2.5.34`（Windows）；默认自动获取 GitHub 最新 Release。
>
...
### 7.4 HTTP 直连（无域名/证书）的功能差异

自托管用户若暂时没有域名/证书，直接通过 `http://<服务器IP>:8080` 访问时，浏览器将其判定为「非安全上下文」，部分 Web API 受限：

| 功能 | HTTP 直连 | 说明 |
| ---- | --------- | ---- |
| 笔记加密 / 解密 | ✅ 正常 | 纯 JS 实现（@noble），不依赖平台 API |
| 同步 / 分享 / 回收站 | ✅ 正常 | — |
| 复制到剪贴板 | ✅ 正常 | 已内置 `execCommand` 降级 |
| PWA 离线缓存 | ❌ 不可用 | Service Worker 需安全上下文 |
| 读取剪贴板（📎 插入） | ❌ 不可用 | 浏览器安全策略，无降级方案 |
| 语音输入 | ❌ 不可用 | Web Speech API 需安全上下文 |

应用会在 HTTP 环境下首次解锁时给出一次性提示。长期使用建议按 §7.1–7.3 配置 HTTPS。

### 7.5 启用 HTTPS 后的客户端配置
