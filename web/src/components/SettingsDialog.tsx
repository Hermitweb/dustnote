...
export function SettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => { void loadConfig().then(c => setApiBase(c.apiBase)); }, []);
  ...
  useEffect(() => {
    ...
    if (!api) return;
    void api.isEnabled().then(setAutostartEnabled).catch(() => undefined);
  }, [desktopEnv]);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" onClick={onClose}>
        ...
        >
          ...
          <div className="max-h-[70vh] space-y-5 overflow-y-auto pr-1">
            {/* 主题 */}
            <div>
              <label className="mb-2 block text-xs font-semibold text-surface-muted">{t('settings.theme')}</label>
                ...
                {THEMES.map((th) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 rounded-lg border-2 p-3 transition-colors ${
                      prefs.theme === th.id ? 'border-mint-500 bg-mint-50 dark:bg-mint-900/30' : 'border-surface-border hover:bg-surface-bg'
                    }`}
                  >
            ...
            {/* 模式 */}
            <div>
              <label className="mb-2 block text-xs font-semibold text-surface-muted">{t('settings.mode')}</label>
                ...
                {(['light', 'dark', 'auto'] as const).map((m) => (
                  <button
                    ...
                    className={`flex-1 rounded-lg border-2 px-3 py-2 text-sm transition-colors ${
                      prefs.mode === m ? 'border-mint-500 bg-mint-50 dark:bg-mint-900/30 text-surface-fg' : 'border-surface-border text-surface-fg hover:bg-surface-bg'
                    }`}
                  >
                    {m === 'light' ? `☀️ ${t('settings.mode_light')}` : m === 'dark' ? `🌙 ${t('settings.mode_dark')}` : `🌓 ${t('settings.mode_auto')}`}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* 语言 */}
            <div>
              <label className="mb-2 block text-xs font-semibold text-surface-muted">{t('settings.language')}</label>
              <div className="flex gap-2">
                {(['zh-CN', 'en'] as const).map((l) => (
                  <button
                    ...
                    className={`flex-1 rounded-lg border-2 px-3 py-2 text-sm transition-colors ${
                      prefs.language === l ? 'border-mint-500 bg-mint-50 dark:bg-mint-900/30 text-surface-fg' : 'border-surface-border text-surface-fg hover:bg-surface-bg'
                    }`}
                  >
                    {l === 'zh-CN' ? '中文' : 'English'}
                  </button>
            ...
            {/* 数据管理 */}
            <div>
              <label className="mb-2 block text-xs font-semibold text-surface-muted">数据管理</label>
            ...
            {/* 服务器地址 */}
            <div>
              <label className="mb-2 block text-xs font-semibold text-surface-muted">🔗 服务器地址</label>
              <div className="flex gap-2">
                <input
                  value={apiBase}
                ...
                />
                <button
                  onClick={() => { saveConfig({ apiBase }); setApiSaved(true); setTimeout(() => setApiSaved(false), 1500); }}
            ...
            {desktopEnv && (
              <div>
                <label className="mb-2 block text-xs font-semibold text-surface-muted">桌面端</label>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className={`text-xs font-semibold ${autostartEnabled ? 'text-mint-700' : 'text-surface-muted'}`}>
            ...
            <div className="rounded-lg border border-surface-border p-3 text-xs text-surface-muted">
              ...
              <div className="font-mono">{t('settings.version')}: {__APP_VERSION__}</div>
