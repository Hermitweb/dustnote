...
export function TemplatePicker({ onClose }: TemplatePickerProps) {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[72vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-surface-border bg-surface-card shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* 头部 */}
        <div className="flex items-center justify-between border-b border-surface-border px-6 py-4">
          <div>
            <h2 className="text-xl font-semibold text-text-primary">{t('templates.title')}</h2>
          ...
          <button
            ...
            className="rounded p-1.5 text-text-tertiary transition-colors hover:bg-surface-3 hover:text-text-primary"
          >
        ...
        {(creating || error) && (
          <div className="border-t border-surface-border px-6 py-3">
            ...
            {error && (
              <p className="text-xs text-danger">{t('templates.error', { reason: error })}</p>
            )}
          </div>
        )}
      </div>
