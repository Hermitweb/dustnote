...
1. 从 [GitHub Releases](https://github.com/Hermitweb/dustnote/releases) 下载 `DustNote_2.4.2_x64-setup.msi`
...
msiexec /i DustNote_2.4.2_x64-setup.msi /qn VELOPACK_INSTALLDIR="D:\Apps\DustNote"
```
...
msiexec /i DustNote_2.4.2_x64-setup.msi /qn

# 静默安装 + 自定义路径
msiexec /i DustNote_2.4.2_x64-setup.msi /qn VELOPACK_INSTALLDIR="D:\Apps\DustNote"

# 静默安装 + 安装日志（便于审计与问题排查）
msiexec /i DustNote_2.4.2_x64-setup.msi /qn /L*v "C:\Logs\dustnote-install.log"
```
...
1. 下载 `DustNote_2.4.2_x64-setup.exe`
2. 双击运行（无需 UAC 提权）
3. 一键安装到 `%LocalAppData%\DustNote\`，自动创建快捷方式并注册到「应用和功能」（HKCU）

```powershell
# 静默安装（无 UI）
DustNote_2.4.2_x64-setup.exe --silent

# 静默安装到指定目录（覆盖默认 %LocalAppData%\DustNote；装 Program Files 需管理员）
DustNote_2.4.2_x64-setup.exe --silent --installto "C:\Program Files\DustNote"

# 启用安装日志
DustNote_2.4.2_x64-setup.exe --silent --log "C:\Logs\dustnote-install.log"
```
...
1. 下载 `DustNote_2.4.2_x64-portable.zip`
2. 解压到任意目录
...
msiexec /x DustNote_2.4.2_x64-setup.msi /qn
...
1. 下载 `DustNote_2.4.2_android.apk`
2. 在手机上打开 APK 文件
3. 允许「安装未知来源应用」（首次需要）
4. 按提示完成安装
