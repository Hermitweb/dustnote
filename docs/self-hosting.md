...
> 适用版本：v2.0.0 及以上
> 完整部署文档请参考 [DEPLOY.md](../DEPLOY.md)（v2.0.0 新增）

本指南帮助你在自己的服务器上部署 DustNote。

> **v2.0.0 双模式架构**：客户端可在「单机模式」下无服务器独立运行。部署服务器后，客户端选择「连接服务器」即可使用联机模式（跨设备同步、在线分享、设备管理等高级功能）。详见 [standalone-mode.md](../.trae/documents/standalone-mode.md)。
