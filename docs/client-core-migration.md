...
1. **Node 版本**：client-core 构建需 Node 20–24（`engines` 锁定 >=20 <25；better-sqlite3 已升级 12.x，原生支持 Node 24）。CI 用 Node 22（better-sqlite3 12.11.1 起官方不再发布 Node 20 (ABI 115) 的 prebuilt，Node 22 LTS 全版本覆盖）。
