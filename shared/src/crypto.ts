 ...
 *
 * v2.5.20：迭代 310000 → 100000。用户真机反馈 310k（原生 JSI）仍达分钟级，
 * 且派生期间 JS 线程冻结导致无法生物识别。100000 是 OWASP 2021 文档的
 * PBKDF2-HMAC-SHA256 推荐档位；个人笔记应用 + 生物识别 + TOTP 多层防护下
 * 安全裕度足够。真机实测原生 100k ≈ 0.3-0.8 秒，满足"1 秒内解锁"目标。
 * 服务端按设备记录 kdf_params，新旧设备各自用创建时的参数派生，互不影响。
 */
export const KDF_PARAMS_MOBILE: KdfParams = {
  ...
  t: 0,
  p: 0,
  iterations: 100000, // OWASP 2021 档位（移动端性能取舍，见上）
  dkLen: 32,
};
